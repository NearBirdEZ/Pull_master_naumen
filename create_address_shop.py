...
class Create_address_shop:
    ...
    def __get_prefix_shop(self):
        prefix = int(input('Введите префикс магазина \n'
                           ...
                           '7 - атак\n'
                           '8 - ашан\n'))
        if prefix in range(1, 9):

            prefixs = ('детский мир', 'алькор и ко', 'тами и ко', 'иль де ботэ', 'леви', 'манго', 'атак', 'ашан')
            return prefixs[prefix - 1]
        else:
            print('Ну ё-маё, давай снова. Используй цифры, которые представлены в диалоговом окне (1-8)')
    ...
    def __full_address(self, name_shop, address):
        ...
        resplaces = (' из ', 'литер', 'литера', ' вл. ', 'кв-л.', 'к.лит.', 'корп.', 'микрорайон', 'мкр-н', 'владение',
                     ...
                     'алькор и ко', 'алькор ', ' ам ', ',', ' фо.', ' мо ', ' м.о ', ' мо.')
        if address.startswith('\"') or address.startswith('\''):
            address = address[1:-1]
        for _ in range(5):
            address = self.__replace_pattern(r'((“)|(\")|(\')|(\«)|(\())[\D\d]*((\")|(\')|(\»)|(\)|(”)))', address)
            address = self.__replace_pattern(r'\d{4, }', address)
        address = self.__replace_pattern(r'[/][\w]*', address)
        address = self.__replace_pattern(r'\w{3,}\d{1,}', address)
